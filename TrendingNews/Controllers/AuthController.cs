...
namespace TrendingNews.Controllers
{
    ...
    public class AuthController : Controller
    {
        ...
        public async Task<IActionResult> LoginWithGoogle([FromBody] string credential)
        {
            ...
            var user = UserList.Where(x => (x.UserName == payload.Name) || (x.UserName == payload.Email)).FirstOrDefault();
            if (payload != null && user == null)
            {
                var newUser = new User { UserName = payload.Email };
            ...
            user = UserList.Where(x => (x.UserName == payload.Name) || (x.UserName == payload.Email)).FirstOrDefault();
